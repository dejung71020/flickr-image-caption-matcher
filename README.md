...
# 시험 3 분석: LangGraph 다단계 검증 시스템

## 1. 시스템 구조

입력 (image, caption)
→ Node 1: Image Analyzer  (이미지 핵심 요소 추출)
→ Node 2: Text Analyzer   (캡션 핵심 요소 추출)
→ Node 3: Judge           (1차 판단)
→ Node 4: Critic          (결과 검토)
→ 조건 분기
    ├─ confidence < 0.60 또는 needs_review=True → Judge 재실행 (최대 2회)
    └─ 조건 미충족 → 종료

Judge에 사용한 프롬프트: 시험2에서 가장 높은 F1을 기록한 프롬프트

---

## 2. 성능 비교 결과

| 시스템 | Accuracy | F1-score | 오류 개수 |
|--------|----------|----------|-----------|
| 단순 1회 판단 (시험1) | 0.8908 | 0.8267 | 13개 |
| LangGraph 다단계 | 0.8431 | 0.7500 | 16개 |
| 차이 | -0.0477 | -0.0767 | +3개 증가 |

- Judge 재실행 발생: 120개 중 84개 샘플 (70%)
- 평균 재실행 횟수: 1.35회
...
### 3-1. LangGraph가 단순 시스템보다 성능이 낮아진 이유

#### ① Critic이 과도하게 재검토를 요구함
120개 중 84개(70%)가 재실행됐다는 것은 Critic이 너무 자주
`needs_review: true`를 반환했음을 의미한다.
올바르게 판단된 샘플도 재검토 대상이 되어 오히려 정답이 뒤집히는
상황이 발생했다.

#### ② LLM의 비결정성(Non-determinism)
LLM은 같은 입력에도 매번 다른 답을 생성한다.
재실행 횟수가 늘어날수록 처음에 맞게 판단한 결과를
다른 답으로 교체할 가능성이 높아진다.
즉, 재실행 자체가 노이즈를 추가하는 역할을 했다.

#### ③ confidence 임계값 0.60이 너무 높음
시험1 결과를 보면 모델은 대체로 높은 confidence로 답하는 경향이 있다.
0.60 미만인 경우가 70%에 달한다는 것은 임계값 설정이 과도하게 보수적이었음을
의미한다. 임계값을 낮추면 불필요한 재실행을 줄일 수 있다.

#### ④ 노드 수 증가에 따른 누적 오류
단계가 많아질수록 각 노드에서 오류가 발생할 가능성이 누적된다.
Image Analyzer나 Text Analyzer가 핵심 요소를 잘못 추출하면
Judge와 Critic 모두 잘못된 정보를 기반으로 판단하게 된다.

---

### 3-2. 복잡한 시스템이 항상 좋은 것은 아니다

이번 실험은 **"복잡한 파이프라인 ≠ 높은 성능"** 을 보여주는 사례다.
LangGraph 같은 다단계 시스템은 아래 조건일 때 효과적이다.

| 효과적인 경우 | 비효과적인 경우 |
|-------------|--------------|
| 단일 모델이 명확히 부족한 경우 | 단일 모델이 이미 충분히 잘 작동하는 경우 |
| 각 노드가 독립적으로 검증 가능한 경우 | Critic이 Judge보다 더 나은 판단을 못 하는 경우 |
| 오류 복구가 명확히 정의된 경우 | 재실행 조건이 과도하게 설정된 경우 |

---

## 4. 개선 방향

1. **confidence 임계값 조정**: 0.60 → 0.40으로 낮춰 불필요한 재실행 감소
2. **Critic 프롬프트 강화**: 단순히 재검토가 아닌, 구체적인 근거가 있을 때만
    `needs_review: true`를 반환하도록 지시
3. **최대 재실행 횟수 감소**: 3회 → 1회로 줄여 노이즈 최소화
4. **앙상블 방식 고려**: 재실행 결과를 교체하는 것이 아닌,
    여러 결과의 다수결로 최종 판단하는 방식

---

## 5. 결론

LangGraph 다단계 시스템은 구조적으로 견고하고 확장 가능한 아키텍처를
제공하지만, 이번 실험에서는 단순 1회 판단 시스템보다 낮은 성능을 보였다.
주요 원인은 Critic의 과도한 재검토 요구와 LLM의 비결정성이다.

다단계 시스템의 도입 여부는 **단순 시스템의 성능이 충분한지**를
먼저 검토한 후 결정하는 것이 바람직하다.